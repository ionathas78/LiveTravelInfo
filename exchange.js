...
/**
 * Runs when the CitySearch page opens.
 * @param {Text} convertFrom Currency code for origin
 * @param {Text} convertTo Currency code for destination
 */
function runCurrencyExchangeQuery(convertFrom, convertTo) {
...
/**
 * Displays currency conversion on CheckCity page
 * @param {Object} apiResponse Response from API query
 */
function returnCurrencyConversion(apiResponse) {
	let inputUSCurrency = $("#us-currency");
	let inputForeignCurrency = $("#foreign-currency");
	...
	inputUSCurrency.val(sourceRate);
	inputForeignCurrency.val(exchangeFloat);
};
...
/**
 * Makes the API call for the Test Page
 * @param {Text} queryString Query to pose to API
 */
function sendAjax_ExchangeTest(queryString) {
    $.ajax({
        method: "GET",
        url: queryString,
    }).then(function (response) {
		returnCurrencyConversionTest(response);
    });
};
...
function getCurrencyConversion(convertFrom, convertTo) {
	...
	sendAjax_ExchangeTest(makeCurrencyLatestQuery(convertFrom));
}

/**
 * Displays Exchange Rate to Test Page
 * @param {Object} apiResponse Response package from API call
 */
function returnCurrencyConversionTest(apiResponse) {
	let conversionRates = apiResponse.conversion_rates;
...
/**
 * Runs when the user clicks the 'Get Currency Exchange' button on the Test Page
 */
